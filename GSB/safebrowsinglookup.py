...
class SafebrowsinglookupClient(object):
    ...
    def lookup(self, *urls):
        ...
        while count * 500 < len(urls):
            ...
            self.__debug("BODY:\n" + body + "\n\n")
	    url = 'https://sb-ssl.google.com/safebrowsing/api/lookup?client=%s&apikey=%s&appver=%s&pver=%s&url=%s' % ('api', self.key, self.version, self.api_version,urls[0])
            #url = 'https://sb-ssl.google.com/safebrowsing/api/lookup?client=%s&key=%s&appver=%s&pver=%s' % ('api', self.key, self.version, self.api_version)
		...
		response = urllib2.urlopen(request).read()
                ...
                if not response_read:
                    self.__debug("No match\n")
	            #print("No Match, Probably Safe")
		    output='safe'
                    results.update( self.__ok(inputs) )
                else:
                    self.__debug("At least 1 match\n")
		    #print"WARNING,Match Found! Category: %s" %response_read
		    output=response
